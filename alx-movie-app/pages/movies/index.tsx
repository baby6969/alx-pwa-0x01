    ...
    <div className="min-h-screen bg-[#110F17] text-white px-4 md:px-10 lg:px-44">
      <div className="py-16">
        <div className="flex flex-col md:flex-row justify-between mb-4 items-center space-x-0 md:space-x-4">
          ...
          <select onChange={(event) => setYear(Number(event.target.value))} className="border-2 border-[#E2D609] outline-none bg-transparent px-4 md:px-8 py-2 mt-4 md:mt-0 rounded-full w-full md:w-auto">
            <option value="">Select Year</option>
            {[2024,2023,2022,2021,2020,2019].map((y) => <option value={y} key={y}>{y}</option>)}
          </select>
        </div>
        ...
        <div className="flex flex-col md:flex-row items-center justify-between">
          ...
          <div className="flex flex-wrap space-x-0 md:space-x-4 mt-4 md:mt-0">
            {['All','Animation','Comedy','Fantasy'].map((g,key) => (<Button title={g} key={key} action={() => setGenre(g)} />))}
          </div>
        </div>
